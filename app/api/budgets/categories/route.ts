...
type LimitChange = { id?: string; limitCents?: number };
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "update-limits") {
      const changes = (body.changes ?? []).map((change) => ({ id: change.id ?? "", limitCents: change.limitCents ?? Number.NaN }));
      await saveBudgetLimits(request, changes);
    ...
    if (body.action === "create") {
      ...
      const item = await createBudgetCategory(request, { scope: body.scope, name: body.name, limitCents: body.limitCents ?? Number.NaN });
