...
export function parseProgressBalance(input: unknown): ProgressBalanceV1 {
  ...
  versionAt(required(record, "contractVersion", path), "$.contractVersion", 1);
  const memberId = nullableIdAt(required(record, "memberId", path), "$.memberId");
  const dimension = enumAt(required(record, "dimension", path), "$.dimension", PROGRESS_DIMENSIONS);
  if (dimension === "household" && memberId !== null) fail("$.memberId", "must be null for household progress");
  if (dimension !== "household" && memberId === null) fail("$.memberId", "is required for personal progress", "missing_field");
  return {
    contractVersion: 1,
    id: idAt(required(record, "id", path), "$.id"),
    householdId: idAt(required(record, "householdId", path), "$.householdId"),
    memberId,
    dimension,
    lifetimePoints: integerAt(required(record, "lifetimePoints", path), "$.lifetimePoints", 0, Number.MAX_SAFE_INTEGER),
    level: integerAt(required(record, "level", path), "$.level", 1, 1_000),
    updatedAt: timestampAt(required(record, "updatedAt", path), "$.updatedAt"),
  };
}
