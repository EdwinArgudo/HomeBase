...
const appearance = JSON.stringify({ species: "marshmallow", palette: "cream", pattern: "plain", accessory: "none" });
const rows = [
...
function fakeDb(sourceRows, unlocks = []) {
  const calls = [];
  return {
    calls,
    prepare(sql) {
            ...
            async all() {
              ...
              const results = sourceRows
                ...
                .map(({ id, household_id, member_id, display_name, base_style_version, appearance_json, active_loadout_json, visibility }) => {
                  ...
                  return { id, member_id, display_name, base_style_version, appearance_json, visibility, equipped_reward_key: verified?.reward_key ?? null };
                });
