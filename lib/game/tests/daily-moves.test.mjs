...
class FakeStatement {
  ...
  async first() {
    assert.match(this.sql, /SELECT MAX\(local_date\)/);
    const [householdId, memberId, localDate] = this.values;
    const dates = this.database.rows
      .filter((entry) => entry.household_id === householdId
        && entry.member_id === memberId
        && entry.local_date < localDate)
      .map((entry) => entry.local_date)
      .sort();
    return { last_local_date: dates.length > 0 ? dates[dates.length - 1] : null };
  }

  async run() {
    assert.match(this.sql, /UPDATE daily_moves SET status = 'expired'/);
    const [householdId, memberId, localDate] = this.values;
    for (const entry of this.database.rows) {
      if (entry.household_id === householdId && entry.member_id === memberId
        && entry.local_date < localDate && ["active", "deferred"].includes(entry.status)) {
        entry.status = "expired";
      }
    }
    this.database.expiries.push([...this.values]);
    return { success: true, results: [], meta: { changes: 0 } };
  }

  async all() {
    // The repetition lookup asks a different question of the same table.
    if (this.sql.includes("SELECT DISTINCT source_id")) {
      assert.match(this.sql, /local_date < \? AND local_date >= \?/);
      assert.equal(this.values.length, 4);
      const [householdId, memberId, localDate, since] = this.values;
      this.database.recentReads.push([...this.values]);
      const seen = new Set(this.database.rows
        .filter((entry) => entry.household_id === householdId
          && entry.member_id === memberId
          && entry.local_date < localDate
          && entry.local_date >= since)
        .map((entry) => entry.source_id));
      return { success: true, results: [...seen].map((source_id) => ({ source_id })), meta: {} };
    }
...
test("a source shown in the last few days is penalised, not banned", async () => {
  const history = [
    row({ slot: 1, local_date: "2026-08-14", source_id: "grocery-a", status: "complete" }),
    row({ slot: 2, local_date: "2026-08-09", source_id: "grocery-old", status: "complete" }),
  ];
  const database = new FakeDatabase(history);
  const window = { householdId: "household-a", memberId: "member-a", localDate: "2026-08-15" };
  await getOrCreateDailyMoveSnapshot(database, window, {
    candidateProvider: async () => [],
    createdAt: () => "2026-08-15T09:00:00.000Z",
    createId: () => "generated",
  });

  assert.equal(database.recentReads.length, 1, "the snapshot asks what was shown recently");
  const [householdId, memberId, localDate, since] = database.recentReads[0];
  assert.deepEqual([householdId, memberId, localDate], ["household-a", "member-a", "2026-08-15"]);
  assert.equal(since, "2026-08-12", "the window looks back three days, not forever");
});

test("returning after time away brings one short move, not a backlog", async () => {
  const database = new FakeDatabase([
    row({ slot: 1, local_date: "2026-08-08", source_id: "task-old", status: "active" }),
    row({ slot: 2, local_date: "2026-08-08", source_id: "task-deferred", status: "deferred" }),
  ]);
  const short = { ...candidate("task-quick"), estimatedSeconds: 60 };
  const long = { ...candidate("task-long"), estimatedSeconds: 900 };

  const moves = await getOrCreateDailyMoveSnapshot(database, scope, {
    candidateProvider: async () => [long, short],
    createdAt: () => "2026-08-16T09:00:00.000Z",
    createId: (context) => `move-${context.candidate.source.id}`,
  });

  assert.equal(moves.length, 1, "one way back in, not three");
  assert.equal(moves[0].source.id, "task-quick", "the gentlest candidate wins");
  assert.equal(moves[0].selectionReasonCode, "comeback");

  assert.equal(database.expiries.length, 1, "the days missed are closed out");
  assert.ok(database.rows
    .filter((entry) => entry.local_date === "2026-08-08")
    .every((entry) => entry.status === "expired"), "nothing from before stays open");
});

test("a member who was here yesterday still gets a full shortlist", async () => {
  const database = new FakeDatabase([
    row({ slot: 1, local_date: "2026-08-15", source_id: "task-yesterday", status: "complete" }),
  ]);
  const moves = await getOrCreateDailyMoveSnapshot(database, scope, {
    candidateProvider: async () => [candidate("task-a"), candidate("task-b"), candidate("task-c")],
    createdAt: () => "2026-08-16T09:00:00.000Z",
    createId: (context) => `move-${context.candidate.source.id}`,
  });

  assert.equal(moves.length, 3);
  assert.ok(moves.every((move) => move.selectionReasonCode !== "comeback"));
});

test("a brand new member is starting out, not coming back", async () => {
  const database = new FakeDatabase([]);
  const moves = await getOrCreateDailyMoveSnapshot(database, scope, {
    candidateProvider: async () => [candidate("task-a"), candidate("task-b")],
    createdAt: () => "2026-08-16T09:00:00.000Z",
    createId: (context) => `move-${context.candidate.source.id}`,
  });

  assert.equal(moves.length, 2, "no history means a normal first day");
  assert.ok(moves.every((move) => move.selectionReasonCode !== "comeback"));
});
