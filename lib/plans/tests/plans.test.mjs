...
const options = (id = "generated-id") => ({ generatedAt: now, createId: () => id });

test("progress logged in Plans is the same progress the goal already counts", async () => {
  const db = await database();

  await applyPlansAction(context(db), { contractVersion: 1, action: "log_goal", id: "goal-mine", value: 1 }, options("entry-one"));
  const after = await applyPlansAction(context(db), { contractVersion: 1, action: "log_goal", id: "goal-mine", value: 2 }, options("entry-two"));

  // Two logs are two entries, and the goal reads their sum alongside the entry
  // the fixture started with.
  assert.equal(after.goals.find((goal) => goal.id === "goal-mine")?.currentValue, 6);
  assert.equal(db.sqlite.prepare("SELECT COUNT(*) AS count FROM goal_entries WHERE goal_id = 'goal-mine'").get().count, 3);
  assert.equal(db.sqlite.prepare("SELECT member_id FROM goal_entries WHERE id = 'entry-two'").get().member_id, "member-a");
});

test("a partner's goal, another household's goal, and a retired goal all refuse to move", async () => {
  const db = await database();

  for (const id of ["goal-partner", "goal-other", "goal-inactive"]) {
    await assert.rejects(
      applyPlansAction(context(db), { contractVersion: 1, action: "log_goal", id, value: 1 }, options()),
      (error) => error instanceof HttpError && error.status === 404,
      `${id} should not be loggable`,
    );
    await assert.rejects(
      applyPlansAction(context(db), { contractVersion: 1, action: "retire_goal", id }, options()),
      (error) => error instanceof HttpError && error.status === 404,
      `${id} should not be retirable`,
    );
  }

  assert.equal(db.sqlite.prepare("SELECT COUNT(*) AS count FROM goal_entries WHERE id = 'generated-id'").get().count, 0);
  assert.equal(db.sqlite.prepare("SELECT COUNT(*) AS count FROM goals WHERE active = 0 AND id != 'goal-inactive'").get().count, 0);
});

test("a new goal belongs to the household or to one person, never to both", async () => {
  const db = await database();

  const shared = await applyPlansAction(context(db), {
    contractVersion: 1, action: "add_goal", text: "Read together", ownership: "shared", trackingType: "sessions", targetValue: 4,
  }, options("goal-new-shared"));
  const added = shared.goals.find((goal) => goal.id === "goal-new-shared");
  assert.equal(added?.name, "Read together");
  assert.equal(added?.currentValue, 0);
  assert.equal(db.sqlite.prepare("SELECT owner_member_id FROM goals WHERE id = 'goal-new-shared'").get().owner_member_id, null);

  await applyPlansAction(context(db), {
    contractVersion: 1, action: "add_goal", text: "My own thing", ownership: "personal", trackingType: "amount", targetValue: 5000,
  }, options("goal-new-mine"));
  const mine = db.sqlite.prepare("SELECT owner_member_id, household_id FROM goals WHERE id = 'goal-new-mine'").get();
  assert.equal(mine.owner_member_id, "member-a");
  assert.equal(mine.household_id, "house-a");
});

test("retiring a goal stops it without erasing what it recorded", async () => {
  const db = await database();

  const after = await applyPlansAction(context(db), { contractVersion: 1, action: "retire_goal", id: "goal-shared" }, options());

  assert.ok(!after.goals.some((goal) => goal.id === "goal-shared"), "it stops being something to do");
  assert.equal(
    db.sqlite.prepare("SELECT COUNT(*) AS count FROM goal_entries WHERE goal_id = 'goal-shared'").get().count,
    2,
    "a goal that is over is not a goal that never happened",
  );
  await assert.rejects(
    applyPlansAction(context(db), { contractVersion: 1, action: "log_goal", id: "goal-shared", value: 1 }, options()),
    (error) => error instanceof HttpError && error.status === 404,
  );
});
