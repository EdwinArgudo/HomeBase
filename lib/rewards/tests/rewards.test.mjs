...
test("a household with nothing new to unlock reads rewards without scanning events", async () => {
  const db = new FakeDb({ progress, events });
  const materialized = await loadAndMaterializeRewards(context(db), now);
  assert.equal(db.batchCalls, 1);

  db.calls.length = 0;
  const settled = await loadAndMaterializeRewards(context(db), "2026-08-16T12:00:00.000Z");
  assert.equal(db.batchCalls, 1, "already-recorded unlocks are not rewritten");
  assert.ok(db.calls.every((call) => !call.sql.includes("FROM game_events")), "the event log is not rescanned");
  assert.deepEqual(
    settled.rewards.map((entry) => entry.unlockedAt),
    materialized.rewards.map((entry) => entry.unlockedAt),
  );
});
...
test("a furnishing belongs to the home and can never be worn", async () => {
  const db = new FakeDb({ progress, events });
  await loadAndMaterializeRewards(context(db), now);
  await assert.rejects(
    equipCurrentPersonaReward(context(db), { contractVersion: 1, rewardKey: "home-lamp" }, { updatedAt: now }),
    (error) => error instanceof HttpError && error.status === 409,
  );
  assert.ok(db.calls.every((call) => !/UPDATE personas[\s\S]*home-lamp/.test(call.sql)));
});
