...
The Vue application is the sole Homebase product shell at `/`. It includes:

- Today, Plans, Ledger, Household, Persona, Adventures, and Display views
...
- Durable D1-backed tasks, groceries, goals, transaction review, personas, progress, rewards, and Minimum Mode
...
Plaid-backed financial accounts, automatic refresh, and repair flows are available when server credentials are configured. See `docs/FIRST_BUILD.md` for accounting rules, security boundaries, and the implementation sequence.
...
<http://localhost:3000>, and a watcher that builds the Vue bundle every product
route serves. Vue edits rebuild in well under a second and the
...
household owner and seeds the initial household. Direct routes such as `/plans`
bootstrap that membership through the focused household summary boundary.
...
### Working on the Vue client

The root application is backed by real household data. For UI work that does
not need a database, the Vue app also runs on its own:
